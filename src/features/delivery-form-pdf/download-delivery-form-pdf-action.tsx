...
import { useDownloadDeliveryFormPdf } from "./use-download-delivery-form-pdf";

type Props = {
  deliveryForm: DeliveryFormDetailDto;
};

export function DownloadDeliveryFormPdfAction({ deliveryForm }: Props) {
  const { downloadPdf, isGenerating } =
    useDownloadDeliveryFormPdf(deliveryForm);

  useKeyboardScope("delivery-form-detail-pdf", {
    F4: () => {
      void downloadPdf();
    },
  });
  ...
  return (
    <Button
      ...
      data-testid="bl-detail-action-pdf"
    >
