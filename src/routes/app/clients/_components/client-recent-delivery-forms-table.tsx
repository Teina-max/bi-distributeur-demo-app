...
export function ClientRecentDeliveryFormsTable({
  rows,
  enabled = true,
}: {
  ...
  const { getRowProps } = useTableKeyboardNav<Row>(normalizedRows, {
    scopeId: "client-recent-delivery-forms",
    enabled,
    onActivate: (row) =>
      void navigate({
  ...
  if (normalizedRows.length === 0) {
    return <Typography variant="muted">Aucun BL pour ce client.</Typography>;
  }
        ...
        {normalizedRows.map((row, index) => {
          ...
          return (
            <TableRow
              ...
              data-testid={`client-delivery-form-row-${row.id}`}
