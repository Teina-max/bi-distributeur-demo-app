...
export function DeliveryFormNewClientAutocomplete({
  selected,
  onPickClient,
  inputRef,
}: Props) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <Label htmlFor="bl-client-input" className="text-xs">
        Client
      </Label>
      <div className="relative">
        <Input
          ...
          data-testid="bl-client-autocomplete-input"
          className="font-mono"
        />
        {enabled && suggestions.length > 0 ? (
          <ul
            ...
            data-testid="bl-client-suggestions-inline"
          >
      ...
      >
        <Command shouldFilter={false}>
          <CommandInput
            ...
            data-testid="bl-client-overlay-input"
          />
          <CommandList>
            ...
            <CommandGroup>
              {overlaySuggestions.map((c) => (
                <CommandItem
                  ...
                  onSelect={() => pick(c)}
                  className="font-mono"
                  data-testid={`bl-client-overlay-item-${c.id}`}
                >
                  <span>
                    {c.code} — {c.name}
                  </span>
