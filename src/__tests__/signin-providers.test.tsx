...
import { SignInCredentialsAndEmailOTP } from "@/routes/auth/signin/sign-in-credentials-and-email-otp";
...
describe("SignInCredentialsAndMagicLinkForm", () => {
  ...
  it("should render email and password fields in credentials mode", async () => {
    ...
    setup(<SignInCredentialsAndEmailOTP callbackUrl="/orgs" />);
  ...
  it("should switch to magic link mode when clicking the link", async () => {
    ...
    const { user } = setup(
      <SignInCredentialsAndEmailOTP callbackUrl="/orgs" />,
    );
  ...
  it("should show error message on failed sign in", async () => {
    ...
    await waitFor(() => {
      expect(toast.error).toHaveBeenCalledWith("Invalid credentials", undefined);
    });
  });
});
...
describe("SignInProvidersContainer", () => {
  it("should render GitHub button when enabled", async () => {
    setup(<SignInProviders providers={["github"]} callbackUrl="/orgs" />);
  ...
  it("should render Google button when enabled", async () => {
    setup(<SignInProviders providers={["google"]} callbackUrl="/orgs" />);
  ...
  it("should render both buttons when both are enabled", async () => {
    setup(
      <SignInProviders providers={["github", "google"]} callbackUrl="/orgs" />,
    );
